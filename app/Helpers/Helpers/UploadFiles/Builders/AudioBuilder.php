...
class AudioBuilder extends BuilderAbstract
{
    // 平台儲存檔案路徑
    ...
    public function __construct(FileService $FileService)
    {
        ...
        if (config('app.storage_name') == 's3') {
            $this->center_file_path_format = sprintf('%s%s',config('app.storage_s3_audio_path'), $this->center_file_path_format);

            $this->client_file_path_format = sprintf('%s%s',config('app.storage_s3_audio_path'), $this->client_file_path_format);
        }
    }
